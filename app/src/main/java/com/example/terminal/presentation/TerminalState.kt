...
data class TerminalState(
    ...
    private val visibleBars: List<Bar>
        get() {
            val startIndex = (scrolledBy / barWidth).roundToInt().coerceAtMost(0)
            val endIndex = (startIndex + visibleBarsCount).coerceAtMost(barList.size)
            return barList.subList(startIndex, endIndex)
        }
    val max : Float get() =  visibleBars.maxOf { it.high }
    val min : Float get() =  visibleBars.minOf { it.low }
    val pxPerPoint : Float get() = terminalHeight / (max - min)
}
